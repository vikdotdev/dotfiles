...
(use-package popper
  :ensure t
  :bind (("C-c p" . my/vterm-popper-toggle))
  :init
  ;; Only vterm buffers are popups (keeps it to “just one popup”: your terminal)
  (setq popper-reference-buffers
        '("^\\*vterm.*\\*$" vterm-mode))

  ;; Popper shows popups at the bottom and allows only one open at a time by default.
  ;; (That’s Popper’s default display behavior.)
  (popper-mode +1)
  (popper-echo-mode +1))  ;; optional echo hints when toggling

;; Close the pane when the terminal exits (vterm kills the buffer on exit)
(setq vterm-kill-buffer-on-exit t)

(defun my/vterm--buffer-name ()
  "Name the vterm buffer (one per project if available)."
  (if (fboundp 'project-current)
      (let* ((proj (project-current nil))
             (root (when proj (car (project-roots proj)))))
        (if root
            (format "*vterm:%s*" (file-name-nondirectory (directory-file-name root)))
          "*vterm*"))
    "*vterm*"))

(defun my/vterm-popper-toggle ()
  "Toggle a bottom vterm popup using Popper.
Creates the vterm buffer if needed, focuses it when shown, and
restores focus when hidden."
  (interactive)
  (let* ((name (my/vterm--buffer-name))
         (buf  (get-buffer name)))
    (cond
     ;; If it's visible, ask Popper to hide the latest popup (this one)
     ((and buf (get-buffer-window buf t))
      (popper-toggle))
     ;; Otherwise create/show it at the bottom and focus it
     (t
      (unless buf
        (setq buf (vterm name)))           ; create the vterm buffer
      ;; Displaying the buffer makes it Popper’s “latest” popup and selects it.
      (display-buffer buf)))))

;; Optional: tweak the popup height (~30% of frame)
;; Popper uses display-buffer under the hood, so you can control height like this:
(add-to-list 'display-buffer-alist
             '("\\*vterm.*\\*"
               (display-buffer-at-bottom)
               (window-height . 0.3)))

(global-set-key (kbd "C-x t p") 'my/vterm-popper-toggle)
#+end_src
* Version control
** Magit
#+begin_src emacs-lisp
