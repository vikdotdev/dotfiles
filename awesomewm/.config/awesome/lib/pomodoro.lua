...
local function update(_awmodoro)
	data[_awmodoro].text:set_markup_silently(data[_awmodoro].elapsed)
...
function awmodoro.draw(_awmodoro, wibox, cr, width, height)
  ...
  wibox.markup = tip
	...
	data[_awmodoro].text:draw(wibox, cr, width, height)
	data[_awmodoro].container:draw(wibox, cr, width, height)
end
...
function awmodoro:begin()
	if not data[self].timer.started then
		...
		data[self].elapsed = 0
    -- data[self].container:set_bg(data[self].colors.active.bg)
		update(self)
...
function awmodoro:pause()
	...
	-- data[self].container:set_bg(data[self].colors.paused.bg)
	update(self)
...
function awmodoro.new(args)
  ...
  local text = wibox.widget({
    markup = 'Pomodoro <b>not running</b>',
    align  = 'center',
    valign = 'center',
    widget = wibox.widget.textbox
  })

  container:set_widget(text)

	data[_awmodoro] = { container = container, colors = colors, width = width, height = height, seconds = seconds, elapsed = 0, timer = timer({ timeout = 1 }), text = text, active_bg_color = active_bg_color, paused_bg_color = paused_bg_color, do_notify = do_notify}
	...
	container:set_bg(colors.active.bg)
	container:set_fg(colors.active.fg)
