...
#+begin_src emacs-lisp
(cond
 ((string= (getenv "DOT_PROFILE") "1") 150)
 ((string= (getenv "DOT_PROFILE") "2") 180)
 ((string= (getenv "DOT_PROFILE") "3") 180)
 (t 150))
#+end_src

#+begin_src emacs-lisp
(setq vik/presentation-mode-flag nil
      vik/current-font-size <<default-font-size>>)
...
(defun vik/toggle-presentation-mode ()
  ...
  (let ((new-size (if (vik/presentation-mode-flag)
                     big-font-size ;; larger size when toggling from default
                   default-font-size))) ;; revert to default or set from profile
    (vik/set-global-font-size new-size)
    (setq vik/current-font-size (if (vik/presentation-mode-flag) <<big-font-size>> <<default-font-size>>))
    (message "Font size set to %d" new-size)))

(vik/set-global-font-size vik/current-font-size)
