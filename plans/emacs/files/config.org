...
(setq org-agenda-scheduled-leaders '("S: " "S: ")
      org-agenda-deadline-leaders  '("D: " "D: "))

(defun my/org-agenda-replace-closed-label ()
  "Replace 'Closed:' with 'C:' and preserve green face."
  (save-excursion
    (goto-char (point-min))
    (let ((case-fold-search nil))
      (while (search-forward "Closed:" nil t)
        (replace-match "C:" t t)
        (add-text-properties
         (- (point) 2) (point)
         '(face org-agenda-done))))))

(add-hook 'org-agenda-finalize-hook #'my/org-agenda-replace-closed-label)
#+end_src
